...
c  set basic parameters
      ...
      open(41,file='BP_para.txt', status='old')
        ...
        read(41,*) window_length_L,lag_lenth_L,cc_min_L
        read(41,*) window_length,lag_lenth,cc_min
        read(41,*) wlen,wdif,nwin
      close(41)
...
c cross correlation parameters
      ...
      nwindl  = window_length_L*nsam ! length of long time window
      nlagl   = lag_lenth_L*nsam     ! number of lags
      cc_minl = cc_min_L             ! minimum cc to eliminate bad data


c other parameters

      ini_ttime=0     !set the beginning of the ttime data
      sta_min = 0.003 !set station minmum distance 
      wstart = pstart-wlen ! set beginning of stacking window
...
c	 if (temp .lt. 67 ) ista_tag(ista)=1    !mark bad recordings
...
c loop for each grid point
     ...
     &*cos((90-xlat1)*pi/180)
         ...
         distc=(180./pi)*acos(dtemp1+dtemp3)          
...
c         call APART (PLAT1, PLON1, stlat(ista),stlon(ista),DIST1,AZ1)
c         dist1=dist1/111.195
...
c          write(*,*)cc_time_shift(n_temp+1),
c     &stanm_cc(n_temp+1)
	  n_temp=n_temp+1     
	 end do  
        close(11)
        ...
        do ista = 1, nsta2
         ...
         nn=0
         do i=1,n_temp
           mtmp=len_trim(stanm(ista))
           csta=stanm(ista)(1:mtmp)
           mtmp=len_trim(stanm_cc(i))
           csta2=stanm_cc(i)(1:mtmp)

           if(csta2 .eq. csta) then
             write(*,*)stanm(i),cc_time_shift(i)
             imax=cc_time_shift(i) 
             nn=1
             exit
           end if
         end do
         if(nn.eq.0 .and. i .gt.n_temp) then
           ista_tag(ista)=1
           cycle
         end if 
...
c          write(file3,*)csta(1:4),'.V'(1:2)  !VERY BAD!!!!!!!!!     
       ...
       write(*,*) 'raw cc has been finished' 
       end if 

c 2_1_st cross correlate with model waveform for 1st grid point

      if(igrid.eq.1)then
        write(*,*)' cross correlate waveforms' 
        nseis3=0  !to trace the starting station for the sum of seis3   
        do ista = 1, nsta2
          do i = 1, ndata
             x(i) = seis2(ista,i)
             xm(i) = seis2(imod,i)
          ...
          call COHERE(XM,X,ISTXM,ISTX,NWINDL,NLAGL,  !mark bad data by long window cc
     1        COHMIN, IMIN, COHMAX,IMAX)
          if (cohmax .lt. cc_minl ) ista_tag(ista)=1  !mark bad data
          if (ista_tag(ista) .eq. 1) cycle           !skip bad data  
...
c          write(*,'(I8,a7,f10.6)')imax,stanm(ista),cohmax
...
c sum waveforms to form the model seismogram

          nseis3=nseis3+1
          if (nseis3 .eq. 1) then
            do i=1,ndata
              seis2(nsta2+1,i)= seis3(ista,i)
            end do
          else
            do i=1,ndata
              seis2(nsta2+1,i)= seis2(nsta2+1,i)+seis3(ista,i)
            end do            
          end if

        end do
c write the model seismogram into sac file
          do i=1,ndata
            x(i) = 1.0*seis2(nsta2+1,i)/nsta2
          end do
c	  if (ista_tag(ista) .eq. 1) cycle  
          csta='Mode'
          stlat(nsta2+1)= xlat1+0.2
          stlon(nsta2+1)= xlon1-0.2
          call setkhv('KSTNM',csta,nerr)
          call setfhv('STLA',stlat(ista),nerr)
          call setfhv('STLO',stlon(ista),nerr)
          file3='Mode.V'
          call wsac0(file3,temp,x,nerr)
...
c 2_2nd cross correlate with model waveform for 1st grid point


      do i_cc=1, 3 !do the stacking three times
      if(igrid.eq.1)then
        write(*,*)' cross correlate waveforms (generating model)' 
        nseis3=0  !to trace the starting station for the sum of seis3   
        do ista = 1, nsta2
          do i = 1, ndata
             x(i) = seis2(ista,i)
             xm(i) = 1.0*seis2(nsta2+1,i)/nsta2
          end do            
...
c	  if (ista_tag(ista) .eq. 1) cycle  
          ...
          call setfhv('STLO',stlon(ista),nerr)
c          file3='Mode.V'
          write(file3, "(a3,i1.1,a2)")'Mod',i_cc,'.V'
          call wsac0(file3,temp,x,nerr)
        write(*,*) i_cc,'th grid match has been finished' 
       end if 
       end do        !end the do loop for the stacking

c 3nd cross correlate with model waveform for 1st grid point
...
c  stack windows
      ...
      do iwin = 1, nwin
         iwin1 = (iwin-1)*wdif/dt + wstart/dt
         iwin2 = iwin1 + nint(wlen/dt)
...
c         write(*,*)iwin1,iwin2
         ...
         do ista = 1, nsta2
           ...
           if (ista_tag(ista) .eq. 1) cycle           !skip bad data
            ...
            ndata3 = iwin2 - iwin1 + 1
c            if(ndata3 .ne. k) write(*,*) 'ndata3 nq k' 
c            amt = 0.1
c            call windo2(x,ndata3,2,amt)
            do j=1,ndata3
              seis3(ista,j) = x(j)
            end do   
         end do
...
c         write(*,*)'stack sum is '
c         write(*,*)tot2

         stacks(igrid, iwin) = tot2

      end do   
         write(*,*) igrid,'th grid finished '
         
      end do

      do iwin = 1, nwin
         twin1 = (iwin-1)*wdif + wstart
         twin2 = twin1 + wlen
         write(8,*)twin1, twin2
         do igrid = 1, ngrid
            write(8,*)glat(igrid), glon(igrid),stacks(igrid,iwin)
         end do
         write(8,*)' '
      end do   
